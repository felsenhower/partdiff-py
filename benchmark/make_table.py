...
def extract_baseline_runtime(df, method: str, col_name: str):
    return ufloat(
        *(
            df.filter(
                pl.col("variant") == "reference", pl.col("method") == method
            ).to_dicts()[0][col_name]
        )
    )
...
def main():
    ...
    df = df.filter(pl.col("i") > 0)
    df = df.group_by(["variant", "method"], maintain_order=True).agg(
    ...
    df = df.select(
        pl.col("variant"),
        pl.col("method"),
        pl.concat_list("runtime_internal_mean", "runtime_internal_std").alias(
            "runtime_internal"
        ),
        pl.concat_list("runtime_total_mean", "runtime_total_std").alias(
            "runtime_total"
        ),
    )
